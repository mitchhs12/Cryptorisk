  ...
  : describe("Setup Unit Tests", async function () {
      ...
      describe("enterLobby", function () {
        it("reverts when you don't pay enough", async function () {
          await expect(setup.enterLobby()).to.be.revertedWith(
            "Send More to Enter Lobby"
          );
        });
      ...
      describe("Testing random word generator", function () {
        it("Calls the vrf coordinator", async function () {
          ...
          await player3_connection.enterLobby({ value: entranceFee });
          await player4_connection.enterLobby({ value: entranceFee });
          //await vrfCoordinatorV2Mock.addConsumer(subscriptionId, setup.address);
          const txResponse = await setup.requestRandomness([]);
